...
describe Atrium::Filter::Facet do
  before(:each) do
    ...
    @exhibit.save
    @filter_facet = Atrium::Filter::Facet.new({:atrium_exhibit_id=>@exhibit.id})
    @filter_facet.save
  end
  ...
  describe "#name" do
    it "should return an empty string if name not set" do
      @filter_facet.name.should == ""
    end
  ...
  describe "#atrium_exhibit" do
    it "atrium exhibit cannot be null" do
      @fail_facet = Atrium::Filter::Facet.new
